...
export default function AdminScholarshipPage() {
    ...
    const fetchSettings = async () => {
        try {
            ...
            const res = await fetch("/api/admin/scholarship/settings");
            if (res.ok) {
                const data = await res.json();
                setSettings({
                    ...data,
    ...
    const handleDeleteApp = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/scholarship/applications?id=${id}`, {
                method: "DELETE"
            });
            if (res.ok) {
                fetchApplications();
    ...
    return (
        <>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-gray-900 tracking-tight">Scholarship Management</h1>
                    <p className="text-sm text-gray-400 font-bold mt-1">Configure portal status and manage student submissions</p>
                </div>
                    ...
                    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
                        <div className="relative w-full md:w-80">
                            ...
                            <input
                                ...
                                className="w-full pl-11 pr-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all"
                            />
                        </div>
                        ...
                        <div className="flex gap-4 w-full md:w-auto self-stretch md:self-auto justify-end">
                            <select
                                ...
                                className="px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold text-gray-700 focus:outline-none focus:border-primary focus:bg-white cursor-pointer"
                            >
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden">
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-100 bg-slate-50/50">
                                            ...
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Student Name</th>
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Phone</th>
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">School Name</th>
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Board</th>
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Exam</th>
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Marks</th>
                                            <th className="text-left px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Income</th>
                                        ...
                                        {filteredApps.map((app, index) => (
                                            <tr key={app._id} className="hover:bg-slate-50/30 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className="text-sm font-bold text-gray-900 block">{app.studentName}</span>
                                                    <span className="text-[10px] text-gray-400 font-bold">By: {app.fullName}</span>
                                                </td>
                                                <td className="px-6 py-4 text-sm text-gray-600 font-semibold">{app.phoneNumber}</td>
                                                <td className="px-6 py-4 text-sm text-gray-500 font-medium max-w-[150px] truncate" title={app.schoolName}>
                                                    {app.schoolName}
                                                </td>
                                                <td className="px-6 py-4 text-sm text-gray-600 font-bold">
                                                    {app.board === "Other" ? app.otherBoard : app.board}
                                                </td>
                                                <td className="px-6 py-4 text-sm text-gray-500 font-medium">{app.examination}</td>
                                                <td className="px-6 py-4 text-sm text-gray-700 font-bold">{app.obtainedMarks}</td>
                                                <td className="px-6 py-4 text-sm text-gray-700 font-bold">₹{app.familyAnnualIncome}</td>
                        ...
                        {!loadingApps && filteredApps.length === 0 && (
                            <div className="text-center py-16">
                                <p className="text-gray-400 font-bold">No scholarship applications found.</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {loadingSettings ? (
                        ...
                        <form onSubmit={handleSaveSettings} className="space-y-6">
                            ...
                            <div className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm hover:bg-slate-50/80 transition-all duration-300">
                                <div>
                                    <h3 className="text-sm font-black text-gray-900 uppercase tracking-wide">Scholarship Applications Status</h3>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <label className="relative inline-flex items-center cursor-pointer select-none group">
                                        ...
                                        />
                                        <div className="w-20 h-11 bg-slate-300 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/20 rounded-full peer peer-checked:after:translate-x-9 after:content-[''] after:absolute after:top-1.5 after:left-1.5 after:bg-white after:rounded-full after:h-8 after:w-8 after:transition-all peer-checked:bg-primary shadow-inner border border-slate-400/30 transition-all duration-300 group-hover:scale-105 active:scale-95"></div>
                                    </label>
                                </div>
                            </div>
                            {/* End Date Picker */}
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    required
                                    className="w-full px-4 py-3 rounded-xl bg-slate-50 border-2 border-slate-100 focus:border-primary focus:bg-white outline-none transition-all font-semibold text-gray-900"
                                />
                            </div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    className="w-full px-4 py-3 rounded-xl bg-slate-50 border-2 border-slate-100 focus:border-primary focus:bg-white outline-none transition-all font-semibold text-gray-900 resize-none"
                                    placeholder="Enter details about target audience, scholarship amount, or instruction banners..."
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full bg-primary hover:bg-orange-600 text-white font-black py-4 rounded-xl shadow-lg hover:shadow-primary/30 transition-all disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer text-sm uppercase tracking-wider"
                            >
                                {savingSettings ? (
                                    <>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-gray-100 animate-scale-in">
                        ...
                        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between shrink-0 bg-white rounded-t-3xl">
                            <div>
                                <h2 className="text-lg font-black text-gray-900 tracking-tight">Application Details</h2>
                        ...
                        <div className="p-6 space-y-6 flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-slate-300">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-2xs">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-700 outline-none focus:border-primary cursor-pointer focus:ring-2 focus:ring-primary/20 transition-all"
                                    >
                                        <option value="Pending">Pending</option>
                                        <option value="Under Review">Under Review</option>
                                        <option value="Approved">Approved</option>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 space-y-4 shadow-sm hover:shadow-md transition-all duration-300">
                                    <h3 className="text-xs font-black text-[#dd7030] uppercase tracking-widest border-l-4 border-[#dd7030] pl-2 mb-2">
                                        Student Information
                                    </h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs">
                                            <span className="text-[9px] text-gray-400 font-bold uppercase block tracking-wider">Student Name</span>
                                            <span className="text-sm font-extrabold text-gray-900 block mt-0.5">{selectedApp.studentName}</span>
                                        </div>
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs">
                                        ...
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs">
                                            <span className="text-[9px] text-gray-400 font-bold uppercase block tracking-wider">Examination</span>
                                            <span className="text-sm font-bold text-gray-700 block mt-0.5">{selectedApp.examination}</span>
                                        </div>
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs sm:col-span-2">
                                            <span className="text-[9px] text-gray-400 font-bold uppercase block tracking-wider">Obtained Marks</span>
                                            <span className="text-sm font-black text-[#dd7030] block mt-0.5">{selectedApp.obtainedMarks}</span>
                                        </div>
                                    </div>
                                </div>

                                {/* Submitter / Personal Profile */}
                                <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 space-y-4 shadow-sm hover:shadow-md transition-all duration-300">
                                    <h3 className="text-xs font-black text-[#dd7030] uppercase tracking-widest border-l-4 border-[#dd7030] pl-2 mb-2">
                                        Submitter & Contact
                                    </h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        ...
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs sm:col-span-2">
                                            ...
                                            <span className="text-sm font-bold text-gray-700 block mt-0.5">{selectedApp.email || "N/A"}</span>
                                        </div>
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs sm:col-span-2">
                                ...
                                <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 space-y-4 shadow-sm hover:shadow-md transition-all duration-300">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        ...
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs">
                                            <span className="text-[9px] text-gray-400 font-bold uppercase block tracking-wider">Mother's Occupation</span>
                                            <span className="text-sm font-bold text-gray-700 block mt-0.5">{selectedApp.motherOccupation}</span>
                                        </div>
                                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-2xs sm:col-span-2">
                                            <span className="text-[9px] text-gray-400 font-bold uppercase block tracking-wider">Family Annual Income</span>
                                            <span className="text-sm font-black text-gray-900 block mt-0.5">₹{selectedApp.familyAnnualIncome}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 space-y-4 shadow-sm hover:shadow-md transition-all duration-300">
                                    ...
                                    <div className="space-y-3">
                                        <button
                                            onClick={() => handleDownload(selectedApp.incomeCertificate, "Income_Certificate")}
                                            className="w-full flex items-center justify-between p-4 bg-white border border-slate-100 hover:border-primary rounded-xl text-xs font-black uppercase tracking-wider text-gray-700 hover:text-primary transition-all cursor-pointer shadow-2xs group"
                                        >
                                            <span className="truncate">Income Certificate</span>
                                            <Download className="w-4 h-4 shrink-0 text-gray-400 group-hover:text-primary transition-colors" />
                                        </button>

                                        <button
                                            onClick={() => handleDownload(selectedApp.resultCopy, "Result_Copy")}
                                            className="w-full flex items-center justify-between p-4 bg-white border border-slate-100 hover:border-primary rounded-xl text-xs font-black uppercase tracking-wider text-gray-700 hover:text-primary transition-all cursor-pointer shadow-2xs group"
                                        >
                                            <span className="truncate">Result Copy</span>
                                            <Download className="w-4 h-4 shrink-0 text-gray-400 group-hover:text-primary transition-colors" />
                                        </button>
                                    </div>
                                </div>
                                ...
                                {selectedApp.whyApply && (
                                    <div className="md:col-span-2 bg-slate-50/50 rounded-2xl p-5 border border-slate-100 space-y-3 shadow-sm hover:shadow-md transition-all duration-300">
                                        <h3 className="text-xs font-black text-[#dd7030] uppercase tracking-widest border-l-4 border-[#dd7030] pl-2">
